...
function Calculator() {
    ...
    const countTotal = data => {
        const total = data.reduce((total, item) => item.radioType === "income" ? total + item.cash : total - item.cash, 0);
        return total;
    }
    ...
    const addHandle = values => {
        setData([...data, values]);
        values.radioType === "expense" ? setTotal(total - values.cash) : setTotal(total + values.cash);
    }
